...
import os
from tdms.custom_filter import *
...
class TdmsAttendanceAdvanceLeaveAddV2View(generics.CreateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = EmployeeAdvanceLeaves.objects.filter(is_deleted=False)
    serializer_class = TdmsAttendanceAdvanceLeaveAddV2Serializer

    @response_modify_decorator_post
    def post(self, request, *args, **kwargs):
        return super().post(request, *args, **kwargs)

class TdmsAttandanceAdminOdMsiReportView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    serializer_class = TdmsAttandanceAdminOdMsiReportSerializer
    ...
    def get(self, request, *args, **kwargs):
        ...
        if month_range:
            ...
            od_emp_list = set(TdmsAttandanceDeviation.objects.filter(
                Q(is_requested=True)&
            ...
            od_details = TdmsAttandanceDeviation.objects.filter(Q(is_requested=True)&
                                                                    Q(from_time__date__gte=month_start)&
                                                                    Q(from_time__date__lte=month_end)&
                                                                    Q(deviation_type='OD')&
        ...
        relative_dir_path = 'media/tdms/attendance/od_mis_report/document'
...
class TdmsAttandanceAdminSAPReportView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    ...
    def get(self, request, *args, **kwargs):
        ...
        if date_range:
            ...
            module_user_list = list(TCoreUserDetail.objects.filter(
                    attendance_type='PMS',user_type__in=('User',)).values_list('cu_user',flat=True))
        ...
        if req_filter:
            ...
            #time.sleep(15)
            request_details = TdmsAttandanceDeviation.objects.filter(
            ...
            fortnightLeaves = TdmsAttandanceFortnightLeaveDeductionLog.objects.filter(
                            attendance__date__date__gte=date_range[0]['month_start__date'],
                            attendance__date__date__lte=date_range[0]['month_end__date'],
            ...
            if fortnightLeaves:
                ...
                for e_fortnightLeaves in fortnightLeaves:
                    ...
                    subtype = None
                    sap_no = None
                    sap_no = TCoreUserDetail.objects.only('sap_personnel_no').get(cu_user=e_fortnightLeaves.attendance.employee).sap_personnel_no
                    if sap_no and sap_no!='#N/A':
                        ...
                        if subtype:
                            data_list_fortnight = [sap_no, subtype, data_str, data_str]
                            # header = ['PERSONNEL NUMBER(SAP ID)',' SUBTYPE',' BEGIN DATE',' ENDDATE']
                            total_list.append(data_list_fortnight)

        # updated by Shubhadeep - follow this code for any file donwload
        relative_dir_path = 'media/tdms/attendance/sap_report/document'
        dir_path = os.path.join(settings.MEDIA_ROOT_EXPORT, relative_dir_path)
        if not os.path.isdir(dir_path):
            os.makedirs(dir_path)
...
class TdmsAttendanceLeaveApprovalListV2View(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    serializer_class = TdmsAttendanceLeaveApprovalListV2Serializer
    pagination_class = CSPageNumberPagination
    queryset = TdmsAttandanceDeviation.objects.filter(Q(is_requested=True)&
                                                        Q(lock_status=False)&
                                                        ~Q(leave_type=None)&
                                                        (Q(approved_status='1')|
                                                        Q(approved_status='5')))
        ...
        if team_approval_flag == '1':
            ...
            if login_user_details.is_superuser == False:
                ...
                if users_list_under_the_login_user:
                    attendence_id_list = TdmsAttendance.objects.filter(
...
class TdmsAttendanceGraceLeaveListModifiedV2View(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsAttandanceDeviation.objects.all()
    ...
    def get(self, request, *args, **kwargs):
        ...
        result = all_leave_calculation_upto_applied_date_tdms(date_object=date_object, user=user)
...
class AbsentAttendanceInsertCronView(APIView):
    ...
    def tdms_att_create(self, filter: dict):
        attendance,_ = TdmsAttendance.objects.get_or_create(**filter)
        return attendance

    def tdms_log_create(self, filter: dict):
        log,_ = TdmsAttandanceLog.objects.get_or_create(**filter)
        return log

    def tdms_request_create(self, filter: dict,user):
        ...
        request,_ = TdmsAttandanceDeviation.objects.get_or_create(**filter)
    ...
    def get(self, request, *args, **kwargs):
        ...
        users_list_in_tdms = TCoreUserDetail.objects.filter(
            ~Q(
                cu_user__in=(TdmsAttendance.objects.filter(date__date=date_time_day,is_deleted=False).values_list('employee',flat=True))
                ),
            (
                Q(
        ...
        print('users_list_in_tdms',users_list_in_tdms,len(users_list_in_tdms))
        
        #raise APIException({'userlist': len(users_list_in_tdms),'ids': users_list_in_tdms, 'query':users_list_in_tdms.query})
        ...
        try:
            with transaction.atomic():
                for each_user in users_list_in_tdms:
                    ...
                    attandance_data = TdmsAttendance.objects.filter(date__date=date_time_day,employee=each_user.cu_user,is_deleted=False)
                    if not attandance_data:
                        
                        project_user_mapping = TdmsProjectUserMapping.objects.filter(user=each_user.cu_user, status=True).order_by('-id').values('project')
                        ...
                        if project_user_mapping:
                            ...
                            project_site = TdmsProject.objects.filter(pk=project).values('id','site_location','site_location__geo_fencing_area')
                            ...
                            if project_site:
                                for e_project_site in project_site:
                                    ...
                                    multi_lat_long=TdmsSiteProjectSiteManagementMultipleLongLat.objects.filter(
                        ...
                        if att_filter:
                            abs_att = self.tdms_att_create(att_filter)
                            print('abs_att',abs_att,type(abs_att))
                            if abs_att:
                                log_filter['attandance'] = abs_att
                                login_log_details = self.tdms_log_create(log_filter)
                                ...
                                if login_log_details:
                                    ...
                                    logout_log_details = self.tdms_log_create(log_filter)
                                    ...
                                    if logout_log_details:
                                        ...
                                        if req_filter:
                                            abs_req = self.tdms_request_create(req_filter,each_user)
...
class AttendanceAddV2View(generics.ListCreateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsAttendance.objects.all()
    ...
    def post(self, request, *args,**kwargs):
        ...
        project_user_mapping = TdmsProjectUserMapping.objects.filter(user=request.user, status=True).order_by('-id').values('project')
        #print("project_user_mapping",project_user_mapping)
        if project_user_mapping:
            project = project_user_mapping[0]['project']
            request.data['user_project_id'] = project
        ...
        if attandance_data:
            TdmsAttandanceLog.objects.get_or_create(
            ...
            attendance_add, created = TdmsAttendance.objects.get_or_create(
...
class AttendanceLogOutV2View(generics.RetrieveUpdateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsAttendance.objects.filter(is_deleted=False)
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            for od in results_data_listofdict:
                ...
                queryset_att_log = TdmsAttandanceLog.objects.filter(attandance=od['id'],login_logout_check__in=('Logout',))
                ...
                if queryset_att_log:
                    ...
                    for e_queryset_att_log in queryset_att_log:
                        ...
                        deviation_details = TdmsAttandanceDeviation.objects.filter(
                            attandance=od['id'],
...
class AttandanceAllDetailsListByPermissonV2View(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsAttendance.objects.filter(
        is_deleted=False,
...
class AttandanceListWithOnlyDeviationByPermissonV2View(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        att_ids = TdmsAttandanceDeviation.objects.filter(attandance__date__date__gte = str(datetime.now().year-1)+'-04'+'-01',
                    ...
                    if queryset:
                        ...
                        if filter:
                            ...
                            if '@' in user_name:
                                ...
                                # print("name", name)
                                if name:
                                    queryset_all = TdmsAttendance.objects.none()
...
class TdmsFlexiTeamFortnightAttendance(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    # pagination_class = CSPageNumberPagination
    queryset = TdmsAttendance.objects.filter(is_deleted=False)
    ...
    def get(self, request, *args, **kwargs):
        ...
        for tuser in tcore_users:
            ...
            for month_master in attendance_month_master:
                ...
                total_hrs_mins, working_hrs_mins, deficit_hrs_mins, leave_deduction, in_number = self.get_tdms_total_working_deficit_hrs_mins(tcore_user=tuser, start_date=month_master.month_start, end_date=month_master.fortnight_date)
                ...
                total_hrs_mins, working_hrs_mins, deficit_hrs_mins, leave_deduction, in_number = self.get_tdms_total_working_deficit_hrs_mins(tcore_user=tuser, start_date=month_master.fortnight_date + timedelta(days=1), end_date=month_master.month_end)
    ...
    def get_tdms_total_working_deficit_hrs_mins(self, tcore_user=None, start_date=None, end_date=None):
        total_hours, working_hours = get_tdms_flexi_hours_for_work_days(tcore_user=tcore_user, start_date=start_date, end_date=end_date)
...
class AttendanceLoginView(generics.ListCreateAPIView):
    ...
    authentication_classes = [TokenAuthentication]
    queryset = TdmsAttendance.objects.all()
    serializer_class = TdmsAttendanceAddSerializer
...
class AttendanceAddView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args,**kwargs):
        ...
        if attandance_data:
            ...
            attendance_add, created = TdmsAttendance.objects.get_or_create(employee=request.user,created_by=request.user,
...
class AttendanceListByEmployeeView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            for od in results_data_listofdict:
                # print("od['id']",od['id'])
                deviation_details = TdmsAttandanceDeviation.objects.filter(attandance=od['id'])
                # print("deviation",deviation_details)      
                login_time = TdmsAttendance.objects.only('login_time').get(id=od['id']).login_time
                logout_time = TdmsAttendance.objects.only('logout_time').get(id=od['id']).logout_time
                total_time = timedelta(hours=00, minutes=00, seconds=00)
...
class AttendanceApprovalList(generics.ListAPIView):
    ...
    authentication_classes = [TokenAuthentication]
    pagination_class = CSPageNumberPagination
    queryset = TdmsAttendance.objects.filter(approved_status=1)
        ...
        if response.data['results']:
            for data in response.data['results']:
                if data['user_project']:
                    ...
                    long_lat_data=TdmsSiteProjectSiteManagementMultipleLongLat.objects.filter(project_site=data['user_project']['site_location']['id']).values('latitude','longitude')
...
class AttandanceAllDetailsListByPermissonView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    # queryset = TdmsAttendance.objects.order_by('-date')
    queryset = TdmsAttendance.objects.filter(is_deleted=False)
...
class AttandanceListWithOnlyDeviationByPermissonView(generics.ListAPIView):
    ...
    def get_queryset(self):
        module_id = self.request.GET.get('module_id', None)
        self.queryset = self.queryset.filter(is_deleted=False,id__in=TdmsAttandanceDeviation.objects.filter(approved_status=4).
...
class AttandanceLogAddView(generics.ListCreateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsAttandanceLog.objects.filter(is_deleted=False)
...
class AttendanceLogEditView(generics.RetrieveUpdateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsAttandanceLog.objects.all()
...
#:::::::::::: Tdms Attandance leave ::::::::::::::::::::::::::::#
# class AdvanceLeaveAddView(generics.ListCreateAPIView):
#     permission_classes = [IsAuthenticated]
#     authentication_classes = [TokenAuthentication]
#     pagination_class = CSPageNumberPagination
#     queryset = TdmsEmployeeLeave.objects.all()
...
class AdvanceLeaveAddView(generics.ListCreateAPIView):
    ...
    authentication_classes = [TokenAuthentication]
    pagination_class = CSPageNumberPagination
    queryset = TdmsEmployeeLeave.objects.filter(approved_status=1)
    serializer_class=AdvanceLeavesAddSerializer
...
class EmployeeConveyanceAddView(generics.ListCreateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsEmployeeConveyance.objects.all()
...
class EmployeeConveyanceListView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsEmployeeConveyance.objects.filter(is_deleted=False)
...
class EmployeeFoodingAddView(generics.ListCreateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsEmployeeFooding.objects.filter(is_deleted=False)
...
class AttandanceListExportView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        attemdance_data = TdmsAttendance.objects.filter(is_deleted=False,**filter).values(
        ...
        for att_data in attemdance_data:
            ...
            deviation_data = TdmsAttandanceDeviation.objects.filter(attandance=att_data['id']).values('from_time','to_time')
            if deviation_data:
...
class TdmsAttandanceSAPExportView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsAttendance.objects.all()
...
class TdmsLiveTrackingForAllEmployeesView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsProjectUserMapping.objects.filter(is_deleted=False,status=True)
    serializer_class = TdmsLiveTrackingForAllEmployeesSerializer
    ...
    def get(self, request, *args, **kwargs):
        ...
        project_id = self.kwargs['project_id']
        data_dict = TdmsProject.objects.filter(pk=project_id).values(
        ...
        data_dict['lat_lng'] = TdmsSiteProjectSiteManagementMultipleLongLat.objects.filter(
            project_site_id=data_dict['site_location_id']).values()
        for data in response.data:
            queryset = TdmsAttandanceLog.objects.filter(
            (
                Q(
            ...
            if queryset:
                ...
                queryset = TdmsAttandanceLog.objects.filter(attandance__employee_id=data['user'],attandance__is_present=True).values('id','attandance','attandance__login_time','time','latitude','longitude','address').order_by('-id')
                if queryset:
                    data['current_position_details'] = queryset[0]
                else:
...
class TdmsLiveTrackingForAEmployeeInAProjectView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsProjectUserMapping.objects.filter(is_deleted=False,status=True)
    serializer_class = TdmsLiveTrackingForAllEmployeesSerializer
    ...
    def get(self, request, *args, **kwargs):
        ...
        data_dict['current_project_details'] = TdmsAttendance.objects.filter(
        ...
        data_dict['current_project_details']['lat_lng'] = TdmsSiteProjectSiteManagementMultipleLongLat.objects.filter(
...
class TdmsAttendanceCronView(APIView):
    ...
    def all_leave_calculation_upto_applied_date(self, date_object=None, user=None,fortnight=None):
        ...
        last_attendance = TdmsAttendance.objects.filter(employee=user.cu_user).values_list('date__date',flat=True).order_by('-date')[:1]
        ...
        availed_grace = TdmsAttandanceDeviation.objects.filter(Q(attandance__employee=user.cu_user) &
    ...
    def get_flexi_hours_calculation(self, date_object=None, user=None,fortnight=None):
        ...
        if fortnight and fortnight == '1':
            ...
            total_hours_1st, working_hours_1st = get_tdms_flexi_hours_for_work_days(tcore_user=user, start_date=fortnight_start_date, end_date=fortnight_end_date,fortnight_flag_cron=True)
            ...
            total_hours_2st, working_hours_2st = get_tdms_flexi_hours_for_work_days(tcore_user=user, start_date=fortnight_start_date, end_date=fortnight_end_date,fortnight_flag_cron=True)
    ...
    def add_fortnight_leave_deduction_log(self,leave_deduction,deviation_in_hours,result,last_attendance_date,employee,fortnight_start_date,fortnight_end_date):
        ...
        attendance_details = TdmsAttendance.objects.filter(
            #is_present=True,
            Q(
            ...
            Q(
                is_present=False,
                pk__in = (
                    TdmsAttandanceDeviation.objects.filter(
    ...
    def insert_leave_data_to_model(self,employee,attendance,leave_type,deviation_type,fortnight_day_remarks):
        duration = TdmsAttandanceDeviation.objects.only('duration').get(attandance=attendance).duration
        _,created = TdmsAttandanceFortnightLeaveDeductionLog.objects.get_or_create(
    ...
    def insert_fortnight_day_remarks_to_attendance(self,employee,attendance,deviation_type,leave_type,fortnight_day_remarks):
        return TdmsAttendance.objects.filter(
    ...
    def auto_approved_hd_fd_by_user(self, date_object=None, user=None):
        deviation_requests = TdmsAttandanceDeviation.objects.filter(
    ...
    def auto_approved_od_by_user(self, date_object=None, user=None,fortnight=str,balance_al=float):
        ...
        deviation_details_pending_reporting_head = TdmsAttandanceDeviation.objects.filter(
            approved_status = 1,
            deviation_type = 'OD',
            attandance__employee=user.cu_user,
            attandance__date__date__lte=date_object,
        ...
        for e_deviation_details in deviation_details_pending_reporting_head:
            ...
            update_auto = TdmsAttandanceDeviation.objects.filter(
        ...
        attendance_ids = TdmsAttandanceDeviation.objects.filter(
            approved_status = 4,
            deviation_type = 'OD',
            attandance__employee=user.cu_user,
            attandance__date__date__lte=date_object,
        ...
        for attendance_id in attendance_ids:
            self.all_leave_calculation_upto_applied_date(date_object=date_object, user=user,fortnight=fortnight)
            duration_length = TdmsAttandanceDeviation.objects.filter(
        ...
        with transaction.atomic():
            if current_date == date_object:
                user_list = TCoreUserDetail.objects.filter(
                    ...
                    ~Q(sap_personnel_no__in=('37200077','37200082','37200085','37200086','37200087','37200091','37200092','37200094','37200098','37200099','37200118','37200120','37200122','37200148','37200149','78200033','80000032','37100001',)),
                    (
                        Q(
                            Q(termination_date__isnull=False)&Q(
                                Q(
...
class TdmsAttendanceLeaveBalanceTransferUserView(APIView):
    permission_classes = [AllowAny]
    ...
    def get_leave_apply_upto_transfer_date(self, attendance_type_present, date_object=None, user=None):
        ...
        leave_details = TdmsAttandanceDeviation.objects.filter(
        ...
        self.insert_to_TdmsAttandanceLeaveBalanceTransferLog(leave_details,user)
    ...
    def insert_to_TdmsAttandanceLeaveBalanceTransferLog(self,leave_details,user):
        for each in leave_details:
            TdmsAttandanceLeaveBalanceTransferLog.objects.get_or_create(
    ...
    def get(self, request, *args, **kwargs):
        ...
        '''
            Note : Leave Balance insert for transfer a user from PMS to HRMS upto transfer date 
                    1) Check TdmsAttandanceDeviation and TdmsAttandance model to check leave which are approved,pending,reject.
