...
class SiteTypeProjectSiteManagementAddView(generics.ListCreateAPIView):
    ...
    queryset = TdmsSiteTypeProjectSiteManagement.objects.filter(is_deleted=False).order_by('-id')
...
class SiteTypeProjectSiteManagementEditView(generics.RetrieveUpdateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsSiteTypeProjectSiteManagement.objects.all()
...
class ProjectSiteManagementSiteAddView(generics.ListCreateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsSiteProjectSiteManagement.objects.filter(is_deleted=False)
    ...
    def list(self, request, *args, **kwargs):
        ...
        for data in response.data:
            ...
            long_lat_data=TdmsSiteProjectSiteManagementMultipleLongLat.objects.filter(project_site=data['id']).values('latitude','longitude')
...
class ProjectSiteManagementSiteDeleteView(generics.RetrieveUpdateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsSiteProjectSiteManagement.objects.all()
...
class ProjectsAddView(generics.ListCreateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsProject.objects.all()
    serializer_class=ProjectsAddSerializer

class ProjectsListView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsProject.objects.filter(is_deleted=False).order_by('-id')
    ...
    def get_queryset(self):
        ...
        if approvals:
            ...
            pre_approvals=TdmsPreExecutionApproval.objects.filter(**approval_dict,is_deleted=False)
            project_id_list=[]
...
class ClosedProjectsListView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsProject.objects.filter(is_deleted=False,state=4).order_by('-id')
    serializer_class=ProjectsListSerializer
    pagination_class = CSPageNumberPagination
    def get_queryset(self):
        filter = {}
...
class ProjectsListCountView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        dic={}
        total_projects = TdmsProject.objects.all().count()
        # print('total_projects', type(total_projects))

        active_projects = TdmsProject.objects.filter(status=True, is_deleted=False).count()
        # print('active_projects',active_projects)

        archived_projects=TdmsProject.objects.filter(status=False).count()
        # print('archived_projects',archived_projects)

        closed_projects=TdmsProject.objects.filter(state=4).count()

        completed_projects=TdmsProject.objects.filter(state=4).count()

        cancelled_projects=TdmsProject.objects.filter(state=5).count()

        pre_execution_projects=TdmsProject.objects.filter(state=1, is_deleted=False).count()
        execution_projects=TdmsProject.objects.filter(state=2, is_deleted=False).count()
        post_execution_projects=TdmsProject.objects.filter(state=3, is_deleted=False).count()
...
class ProjectsEditView(generics.RetrieveUpdateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        response=dict()
        project = TdmsProject.objects.filter(is_deleted=False,pk=kwargs['pk'])
        ...
        for e_project in project:
            ...
            bidder_type=TdmsTenderBidderType.objects.filter(tender=e_project.tender.id,is_deleted=False)
            ...
            m_list = list()
            e_list = list()
            machinary_list = TdmsProjectMachinaryMapping.objects.filter(project=e_project.id,
                                                                        is_deleted=False)
            ...
            employee_list = TdmsProjectUserMapping.objects.filter(project=e_project.id, is_deleted=False)
...
class ProjectsManpowerReassignTransferView(generics.CreateAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsProjectUserMapping.objects.all()
...
class ProjectStartToEndDateListView(generics.RetrieveAPIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [TokenAuthentication]
    queryset = TdmsProject.objects.filter(is_deleted=False)
    serializer_class=ProjectsListCountSerializer
...
class EmployeeListBySiteProjectListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        filter = {
        'pk__in':TdmsProjectUserMapping.objects.filter(
            project_id__in=(TdmsProject.objects.filter(site_location_id=site_id).values_list('id',flat=True))).values_list('user',flat=True)
        }
...
class ProjectSitesByLoginUserView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        user_projects = TdmsProjectUserMapping.objects.filter(user=login_user).values_list('project__site_location',flat=True)
...
class ProjectByLoginUserView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        login_user = self.request.user
        user_projects = TdmsProjectUserMapping.objects.filter(user=login_user).values_list('project',flat=True)
        #print('user_projects',user_projects)
        queryset = self.queryset.filter(pk__in=user_projects)
        return queryset
