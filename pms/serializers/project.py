    ...
    def update(self, instance, validated_data):
        try:
            with transaction.atomic():
                ...
                daily_expence_configuration_for_pm = PmsDailyExpenceApprovalConfiguration.objects.filter(level='Project Manager',project=instance)
                ...
                daily_expence_configuration_for_pc = PmsDailyExpenceApprovalConfiguration.objects.filter(level='Project Coordinator',project=instance,)
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                p_list = list()
                PmsProjectUserMapping.objects.filter(user=validated_data.get("user")).delete()
...
class ProjectsMachinaryReassignTransferSerializer(serializers.ModelSerializer):
    created_by = serializers.CharField(default=serializers.CurrentUserDefault())
    project_list = serializers.ListField(required=False)
    

    class Meta:
        model = PmsProjectsMachinaryMapping
        fields = ('id', 'created_by', "project_list","machinary")
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                project_list = validated_data.get("project_list")
                p_list = list()
                PmsProjectsMachinaryMapping.objects.filter(machinary=validated_data.get("machinary")).delete()
                for p_l in project_list:
                    start_date = p_l['start_date']
                    expire_date = p_l['expire_date']
                    start_date = datetime.datetime.strptime(start_date, "%Y-%m-%dT%H:%M:%S.%fZ")
                    ...
                    MachinaryMapping = PmsProjectsMachinaryMapping.objects. \
                        create(
                        project_id=p_l["project"],
                        machinary=validated_data.get("machinary"),
                        machinary_s_d_req=start_date,
                        machinary_e_d_req=expire_date,
                        owned_by=validated_data.get("created_by"),
                        created_by=validated_data.get("created_by")
                    )
                    project_dict = {}
                    project_dict["id"] = MachinaryMapping.id
                    project_dict["project"] = MachinaryMapping.project.id
                    project_dict["machinary"] = MachinaryMapping.machinary.id
                    project_dict["start_date"] = MachinaryMapping.machinary_s_d_req
                    project_dict["expire_date"] = MachinaryMapping.machinary_e_d_req
                    p_list.append(project_dict)
                
                response["machinary"] = validated_data.get("machinary")
                response["project_list"] = p_list
                return response
